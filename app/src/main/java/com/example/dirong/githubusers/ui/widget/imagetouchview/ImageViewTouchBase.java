...
public abstract class ImageViewTouchBase extends ImageView implements IDisposable {
    ...
    public interface OnDrawableChangeListener {
        ...
        /**
         * Callback invoked when drawable set and custom zoom needed
         */
        void onCustomZoomRequest();
    }
    ...
    /**
     * Use this to change the ImageViewTouchBase.DisplayType of
     * this View
     *
     * @author alessandro
     */
    public enum DisplayType {
        ...
        FIT_IF_BIGGER,
        /**
         * For image will applied custom scale
         */
        USE_CUSTOM_SCALE
    }
    ...
    public void setMinCustomScale(float minCustomScale) {
        this.minCustomScale = minCustomScale;
    }

    public void setMaxCustomScale(float maxCustomScale) {
...
//    }
    ...
    private void setMinScale(float value) {
        if (LOG_ENABLED) {
    ...
    private float getMinScaleForImage() {
        final Drawable drawable = getDrawable();
        if (drawable == null) {
            return 1F;
        }
    ...
    protected void _setImageDrawable(final Drawable drawable, final Matrix initial_matrix, float min_zoom, float max_zoom) {
        ...
        if (min_zoom != ZOOM_INVALID && max_zoom != ZOOM_INVALID) {
            ...
            if (mScaleType == DisplayType.USE_CUSTOM_SCALE && mDrawableChangeListener != null) {
                mDrawableChangeListener.onCustomZoomRequest();
            }
            mMinZoom = ZOOM_INVALID;
            mMaxZoom = ZOOM_INVALID;
    ...
    public void applyMinZoomLevel() {
        setMinScale(getMinScale());
        zoomTo(getMinScale(), 1);
    }
     ...
     * or the {@link com.togethernetworks.imageviewtouch.ImageViewTouchBase.DisplayType} has been modified
     *
     * @param left
     * @param top
     * @param right
    ...
    protected float computeMaxZoom() {
        ...
        float scale = Math.max(fw, fh) * 2 * density * density;
    ...
    protected void zoomTo(float scale, float centerX, float centerY) {
        if (getMinScale() >= getMaxScale()) return;
        if (scale > getMaxScale()) scale = getMaxScale();
